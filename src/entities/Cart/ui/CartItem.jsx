...
const CartItem = ({ order, img, title, price, size, quantity }) => {
  const { removePizza } = useCartContext();
      ...
      <div className="flex items-center gap-4 w-full justify-between">
        <div className="flex items-center gap-4 w-full justify-between">
          ...
          <div className="flex flex-col items-center gap-4">
            <Button
              title={'+'}
              onClick={() => increasePizza(order)}
              className={'!p-0 size-10 flex items-center'}
              disabled={quantity >= 10}
            />
            <p className="text-xl dark:text-white text-green-950">{quantity}</p>
            <Button
              title={'-'}
              onClick={() => decreasePizza(order)}
              className={'!p-0 size-10 flex items-center'}
            />
          </div>
        </div>
        ...
        <Button
          title={<DeleteOutlined style={{ fontSize: '2rem' }} />}
          className={'!flex items-center !p-2'}
          onClick={() => removePizza(order)}
        />
      </div>
    </div>
  );
